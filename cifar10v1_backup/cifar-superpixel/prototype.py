...
def train_model(
    ...
    with open("log.txt", "a") as log_file:
        log_file.write("Processing images into graphs took {ptime}s\n".format(ptime=ptime))
    ...
    dset_train_loader, dset_valid_loader = map(
        lambda ds: torch.utils.data.DataLoader(ds, batch_size=batch_size, shuffle=True, num_workers=4, collate_fn=util.batch_graphs, pin_memory=PIN_MEMORY),
...
def test_model(
    ...
    dset_test_loader = torch.utils.data.DataLoader(test_dset, batch_size=8, shuffle=True, num_workers=4, collate_fn=util.batch_graphs, pin_memory=PIN_MEMORY)
...
def main(
    ...
    if use_cuda:
        print("Using CUDA")
        print("CUDA version: {version}".format(version=torch.version.cuda))
    else:
