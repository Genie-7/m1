...
def get_graph_from_image(PIL_image, desired_nodes=75):
    ...
    segments = slic(image, n_segments=desired_nodes, compactness=10, sigma=1, start_label=0)
    
    num_nodes = np.max(segments) + 1
    nodes = {node: {"rgb_list": [], "pos_list": []} for node in range(num_nodes)}
    ...
    for node in nodes:
        ...
        features = np.concatenate([
            rgb_mean, rgb_std, pos_mean, pos_std
        ])
        G.add_node(node, features=list(features))
    ...
    n = len(G.nodes)
    m = len(G.edges)
    h = np.zeros([n, 10]).astype(np.float32)  # 10 features: 3 (rgb_mean) + 3 (rgb_std) + 2 (pos_mean) + 2 (pos_std)
    edges = np.zeros([2*m, 2], dtype=np.int64)
