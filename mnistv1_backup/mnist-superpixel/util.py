...
def get_graph_from_image(image, desired_nodes=75):
    #print(f"Starting get_graph_from_image with image shape: {image.shape}")
    ...
    # print(f"Number of unique segments: {num_nodes}")
    # print(f"Unique segment values: {unique_segments}")
    # print(f"Min segment value: {np.min(asegments)}")
    # print(f"Max segment value: {np.max(asegments)}")
    
    # Create nodes dictionary based on actual segment values
    nodes = {
        node: {
            "rgb_list": [],
            "pos_list": []
    ...
    segment_counts = {i: 0 for i in unique_segments}
    ...
    for y in range(height):
        for x in range(width):
            ...
            segment_counts[node] += 1
    
    #print("Finished populating nodes")
    #print("Segment counts:")
    # for node, count in segment_counts.items():
    #     print(f"Node {node}: {count} pixels")
    
    G = nx.Graph()
    for node in nodes:
        # print(f"Processing node {node}")
        # print(f"RGB list length: {len(nodes[node]['rgb_list'])}")
        # print(f"Pos list length: {len(nodes[node]['pos_list'])}")
        
        if len(nodes[node]["rgb_list"]) == 0:
            print(f"Warning: Empty RGB list for node {node}")
            continue
    ...
    num_features = len(list(G.nodes[list(G.nodes)[0]]["features"]))  # Get the number of features from the first node
    h = np.zeros([len(G.nodes), num_features]).astype(NP_TORCH_FLOAT_DTYPE)
    edges = np.zeros([2*len(G.edges), 2]).astype(NP_TORCH_LONG_DTYPE)
    
    # print("Populating edges array...")
    for e, (s, t) in enumerate(G.edges):
        edges[e, 0] = s - 1  # Subtract 1 to convert to 0-indexing
        edges[e, 1] = t - 1
        edges[len(G.edges) + e, 0] = t - 1
        edges[len(G.edges) + e, 1] = s - 1
    
    # print("Populating features array...")
    for i, node in enumerate(G.nodes):
        h[i, :] = G.nodes[node]["features"]
    
    del G
    # print("Finished get_graph_from_image")
